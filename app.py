...
df['name'] = df['name'].apply(brand_name)

name = st.selectbox("Select Car Brand", df["name"].unique())
year = st.slider("Manufactured Year", 1994, 2026)
km_driven = st.slider("No of KMs Driven", 0, 2000000)
...
mileage = st.slider("Mileage (in kmpl)", 10.0, 50.0)
engine = st.slider("Engine (in CC)", 500, 5000)
max_power = st.slider("Max Power (in bhp)", 20, 500)
